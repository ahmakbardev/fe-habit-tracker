...
export default function TaskFolderDashboard({
  folders,
  onSelect,
  onRenameFolder,
  onDeleteFolder,
    ...
    <div className="relative flex-1 h-full w-full bg-slate-50 p-10 overflow-y-auto">
      <div className="mx-auto">
        <div className="flex items-end justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-2">Select Task Folder</h1>
            <p className="text-slate-500">Choose a folder to view your task projects.</p>
          </div>
        ...
        <div className={clsx(viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" : "flex flex-col gap-2")}>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 flex-1">
                  <div className={clsx("bg-blue-50 text-blue-600 rounded-xl transition-colors group-hover:bg-blue-500 group-hover:text-white", viewMode === "grid" ? "p-3" : "p-2")}>
                    <Icon className={clsx(viewMode === "grid" ? "w-6 h-6" : "w-5 h-5")} />
                  </div>
                  <div>
                    <h3 className={clsx("font-semibold text-slate-800 group-hover:text-blue-700 transition-colors", viewMode === "grid" ? "text-base" : "text-sm")}>
                      {folder.name}
                    </h3>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={clsx("transition-all", viewMode === "grid" ? "absolute top-6 right-6 opacity-0 group-hover:opacity-100" : "opacity-0 group-hover:opacity-100")} onClick={(e) => e.stopPropagation()}>
                    <ItemActionMenu
                      ...
                      itemType="Folder"
                      onRename={(newName) => onRenameFolder(folder.name, newName)}
          ...
          <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
            <PopoverTrigger asChild>
              <button className={clsx("group flex items-center justify-center border-2 border-dashed border-slate-200 hover:border-blue-400 hover:bg-blue-50/50 transition-all text-slate-400 hover:text-blue-500", viewMode === "grid" ? "flex-col gap-3 rounded-2xl py-8 h-full" : "flex-row gap-3 px-4 py-3 rounded-xl w-full")}>
            ...
            <PopoverContent className="w-64 p-3" align="center">
              <div className="space-y-2">
                <h4 className="font-medium text-sm text-slate-900">New Task Folder</h4>
                <div className="flex gap-2">
                  <input autoFocus value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleCreateSubmit()} placeholder="Folder name..." className="flex-1 text-sm border rounded-md px-2 py-1.5 outline-none focus:ring-1 focus:ring-blue-500" />
                  <button onClick={handleCreateSubmit} disabled={!newFolderName.trim()} className="bg-black text-white p-1.5 rounded-md hover:opacity-80 disabled:opacity-50"><Check className="w-4 h-4" /></button>
                </div>
              </div>
            </PopoverContent>
          </Popover>
