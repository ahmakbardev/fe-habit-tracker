...
export default function ItemActionMenu({
  itemName,
  itemType = "Item", // Sekarang ini valid karena "Item" sudah ada di type Props
  onRename,
  onDelete,
  align = "end",
  triggerClassName,
  ...
  const handleRenameSubmit = () => {
    if (tempName.trim() && tempName !== itemName) {
      onRename(tempName);
      setOpen(false);
    }
  };
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent
        align={align}
        className="w-48 p-2 bg-white border shadow-lg rounded-xl z-50"
        onClick={(e) => e.stopPropagation()}
      >
        {/* VIEW: MENU UTAMA */}
        {view === "menu" && (
        ...
        {view === "delete" && (
          <div className="flex flex-col gap-2 p-1">
            <div className="flex items-center gap-2 text-red-600 mb-1">
              ...
              <span className="text-xs font-bold">Delete {itemType}?</span>
            </div>
