...
import { Layers, LucideIcon, Hash } from "lucide-react";
...
import { NoteService, ApiWorkspace, ApiFolder, ApiNote } from "../services/note-service";
...
export default function NotesClientWrapper() {
  ...
  // --- 2. DATA STATE ---
  const [workspaces, setWorkspaces] = useState<{ id: string; name: string; icon: LucideIcon }[]>([]);
  const [folderIcons, setFolderIcons] = useState<Record<string, LucideIcon>>({});
  ...
  const activeFolderName = useMemo(() =>
    activeFolderId ? folderNames[activeFolderId] || "" : "",
  [folderNames, activeFolderId]);

  const currentFolders = useMemo(() => {
    if (!activeWorkspaceId || !notesData[activeWorkspaceId]) return [];
    return Object.keys(notesData[activeWorkspaceId]).map(id => ({
      id,
      name: folderNames[id] || "Untitled Folder",
      icon: folderIcons[id] || Hash
    }));
  }, [activeWorkspaceId, notesData, folderNames, folderIcons]);

  const currentNotes = useMemo(() => {
    if (!activeWorkspaceId || !activeFolderId) return [];
    return notesData[activeWorkspaceId]?.[activeFolderId] || [];
  }, [activeWorkspaceId, activeFolderId, notesData]);
  ...
  const navigateTo = useCallback((params: { workspace?: string | null, folder?: string | null, note?: string | null }, replace = false) => {
    ...
    const isDeeper =
      (!oldParams.workspace && params.workspace) ||
      (oldParams.workspace && !oldParams.folder && params.folder) ||
      (oldParams.folder && !oldParams.note && params.note);
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const formattedWorkspaces = data.map(w => ({
        id: w.id,
        name: w.name,
        icon: w.icon_name ? getIconByName(w.icon_name) : Layers
      }));
      setWorkspaces(formattedWorkspaces);

      const newNotesData: Record<string, Record<string, NoteItem[]>> = {};
      const newFolderNames: Record<string, string> = {};
      const newFolderIcons: Record<string, LucideIcon> = {};

      data.forEach((workspace: ApiWorkspace) => {
        newNotesData[workspace.id] = {};
        workspace.folders.forEach((folder: ApiFolder) => {
          newFolderNames[folder.id] = folder.name;
          if (folder.icon_name) {
            newFolderIcons[folder.id] = getIconByName(folder.icon_name);
          }
          newNotesData[workspace.id][folder.id] = folder.notes.map((note: ApiNote) => ({
  ...
  const createNote = useCallback(async () => {
    ...
    try {
      const newNoteApi = await NoteService.createNote(activeFolderId, "Untitled Note");
      const newNote: NoteItem = {
        id: newNoteApi.id,
        folder_id: newNoteApi.folder_id,
      ...
      setNotesData(prev => ({
        ...
        [activeWorkspaceId]: {
          ...
          [activeFolderId]: [newNote, ...(prev[activeWorkspaceId]?.[activeFolderId] || [])]
        }
      }));
  ...
  }, [activeWorkspaceId, activeFolderId, extractContentHtml, navigateTo]);

  const deleteNote = useCallback(async (noteId: string) => {
    try {
      await NoteService.deleteNote(noteId);
      setNotesData(prev => {
        if (!activeWorkspaceId || !activeFolderId) return prev;
        const folderNotes = prev[activeWorkspaceId][activeFolderId] || [];
        return {
          ...prev,
          [activeWorkspaceId]: {
            ...prev[activeWorkspaceId],
            [activeFolderId]: folderNotes.filter(n => n.id !== noteId)
          }
        };
      });
  ...
  }, [activeWorkspaceId, activeFolderId, activeNoteId, navigateTo]);
  ...
  const duplicateNote = useCallback(async (note: NoteItem) => {
    ...
    try {
      ...
      setNotesData(prev => ({
        ...
        [activeWorkspaceId]: {
          ...prev[activeWorkspaceId],
          [activeFolderId]: [copy, ...(prev[activeWorkspaceId]?.[activeFolderId] || [])]
        }
      }));
    } catch (error) {
      console.error("Failed to duplicate note:", error);
    }
  }, [activeWorkspaceId, activeFolderId, extractContentHtml]);
    ...
    try {
      ...
      setNotesData(prev => {
        ...
        return {
          ...
          [activeWorkspaceId]: {
            ...prev[activeWorkspaceId],
            [activeFolderId]: prev[activeWorkspaceId][activeFolderId].map(n =>
  ...
  }, [activeWorkspaceId, activeFolderId, stripHtml]);
  ...
  const handleCreateWorkspace = useCallback(async (name: string) => {
    try {
      const newWs = await NoteService.createWorkspace(name);
      setWorkspaces(prev => [...prev, {
        id: newWs.id, name: newWs.name,
        icon: newWs.icon_name ? getIconByName(newWs.icon_name) : Layers
      }]);
      setNotesData(prev => ({ ...prev, [newWs.id]: {} }));
      navigateTo({ workspace: newWs.id, folder: "", note: null });
  ...
  const handleRenameWorkspace = useCallback(async (id: string, name: string) => {
    try {
      ...
      await NoteService.updateWorkspace(id, name, workspace?.icon ? getIconName(workspace.icon) : undefined);
  ...
  const handleDeleteWorkspace = useCallback(async (id: string) => {
    try {
      ...
      if (activeWorkspaceId === id) navigateTo({ workspace: null, folder: null, note: null }, true);
    } catch (e) { console.error(e); }
  }, [activeWorkspaceId, navigateTo]);

  const handleCreateFolder = useCallback(async (name: string, icon: LucideIcon) => {
    if (!activeWorkspaceId) return;
    try {
      const iconName = getIconName(icon);
      const newFolder = await NoteService.createFolder(activeWorkspaceId, name, iconName);
      setFolderNames(prev => ({ ...prev, [newFolder.id]: name }));
      setFolderIcons(prev => ({ ...prev, [newFolder.id]: icon }));
      setNotesData(prev => ({
        ...prev,
        [activeWorkspaceId]: { ...prev[activeWorkspaceId], [newFolder.id]: [] }
      }));
      navigateTo({ folder: newFolder.id, note: null });
    } catch (e) { console.error(e); }
  }, [activeWorkspaceId, navigateTo]);
  ...
  const handleRenameFolder = useCallback(async (id: string, name: string) => {
    try {
      ...
      await NoteService.updateFolder(id, name, icon ? getIconName(icon) : undefined);
  ...
  const handleDeleteFolder = useCallback(async (id: string) => {
    try {
      ...
      setNotesData(prev => {
        if (!activeWorkspaceId) return prev;
        const newFolderData = { ...prev[activeWorkspaceId] };
        delete newFolderData[id];
        return { ...prev, [activeWorkspaceId]: newFolderData };
      });
      if (activeFolderId === id) navigateTo({ folder: null, note: null }, true);
    } catch (e) { console.error(e); }
  }, [activeWorkspaceId, activeFolderId, navigateTo]);
        ...
        >
          <div className={isSidebarCollapsed ? "hidden" : "h-full w-full overflow-hidden"}>
            <NotesSidebar
              ...
              customIcons={folderIcons}
              onWorkspaceSelect={(id) => navigateTo({ workspace: id, folder: "", note: null })}
              onFolderSelect={(id) => navigateTo({ folder: id, note: null })}
        ...
        {loading && workspaces.length === 0 ? (
          ...
          <>
            {!activeWorkspaceId && (
              <WorkspaceDashboard
                workspaces={workspaces}
                onSelect={(id) => navigateTo({ workspace: id, folder: "", note: null })}
                onRenameWorkspace={handleRenameWorkspace}
                onDeleteWorkspace={handleDeleteWorkspace}
                onCreateWorkspace={handleCreateWorkspace}
              />
            ...
            {activeWorkspaceId && activeFolderId === "" && (
              <FolderDashboard
                folders={currentFolders}
                onSelect={(id) => navigateTo({ folder: id, note: null })}
                onRenameFolder={handleRenameFolder}
                onDeleteFolder={handleDeleteFolder}
                onBack={() => navigateTo({ workspace: null, folder: null, note: null })}
                onCreateFolder={handleCreateFolder}
              />
            )}

            {activeFolderId !== "" && (!isMobile || !selectedNote) && (
              <div
              ...
              >
                {isContentCollapsed ? (
                  ...
                  <NotesContentPanel
                    ...
                    onBack={() => navigateTo({ folder: null, note: null })}
                    onRenameFolder={handleRenameFolder}
